    ...
    def configure_optimizers(self):
        ...
        return {
            ...
            'lr_scheduler': ReduceLROnPlateau(
                optimizer, mode='min', verbose=True, factor=config.scheduler_factor, patience=config.patience,
...
class Phase2Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = (self.criterion(logits, y) * (1 - config.alpha) + self.criterion(logits, y2) * config.alpha) * (
                    config.cell_reso + 1)
        self.log('train_loss', loss, prog_bar=True, on_step=True, on_epoch=True, sync_dist=True)
        return loss
...
if __name__ == "__main__":
    ...
    # x = torch.randn(2, 1, 4, 256)  # batchsize=2
    # model = PhaseModel()
    # y = model(x)
    # print(y.shape)
    ...
    model = PhaseModel()
    y = model(x)
    print(y.shape)  # (2, config.cell_reso+1)
